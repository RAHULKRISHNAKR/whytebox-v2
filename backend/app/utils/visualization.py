...
class ExplainabilityVisualizer:
    ...
    def create_heatmap_overlay(
        ...
        # Blend
        overlay = (
            ((1 - alpha) * image.astype(np.float32) + alpha * colored.astype(np.float32))
            .clip(0, 255)
            .astype(np.uint8)
        )
    ...
    def create_side_by_side(
        original: np.ndarray, heatmap: np.ndarray, overlay: np.ndarray, padding: int = 10
    ) -> np.ndarray:
        """
    ...
    def create_comparison_grid(
        original: np.ndarray, results: dict, padding: int = 10
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def save_visualization(image: np.ndarray, output_path: str, format: str = "PNG") -> None:
        """
    ...
    @staticmethod
    def create_attention_map(heatmap: np.ndarray, threshold: float = 0.5) -> np.ndarray:
        """
    ...
    def create_bounding_box(
        image: np.ndarray, heatmap: np.ndarray, threshold: float = 0.5
