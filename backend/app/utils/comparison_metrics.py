...
class ExplainabilityMetrics:
    ...
    def compute_sparsity(attribution_map: np.ndarray, threshold: float = 0.1) -> float:
        ...
        """
        # Normalize to [0, 1]
        normalized = (attribution_map - attribution_map.min()) / (
            attribution_map.max() - attribution_map.min() + 1e-7
        )
    ...
    def compute_similarity(
        map1: np.ndarray, map2: np.ndarray, method: str = "correlation"
    ) -> float:
        """
    ...
    @staticmethod
    def rank_methods(
        attribution_maps: Dict[str, np.ndarray], weights: Dict[str, float] = None
    ...
    @staticmethod
    def compare_all_metrics(attribution_maps: Dict[str, np.ndarray]) -> Dict[str, Dict[str, float]]:
        """
        ...
        for i, name1 in enumerate(method_names):
            for name2 in method_names[i + 1 :]:
                similarity = ExplainabilityMetrics.compute_similarity(
                    attribution_maps[name1], attribution_maps[name2], method="correlation"
                )
    ...
    @staticmethod
    def generate_comparison_report(attribution_maps: Dict[str, np.ndarray]) -> Dict[str, any]:
        """
        ...
        report = {
            ...
            "metrics": metrics,
            "ranking": [{"method": name, "score": score} for name, score in ranking],
