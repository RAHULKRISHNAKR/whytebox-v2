    ...
    async def validate(self, file) -> None:
        ...
        if not any(file.filename.endswith(ext) for ext in self.allowed_extensions):
            raise ValidationAPIError(
                ...
                details={"allowed_extensions": self.allowed_extensions, "filename": file.filename},
            )
        ...
        if size == 0:
            raise ValidationAPIError(message="File is empty", details={"filename": file.filename})
...
class JSONValidator:
    ...
    def __init__(self, max_depth: int = 10, max_keys: int = 100, max_string_length: int = 10000):
