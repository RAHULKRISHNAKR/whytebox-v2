...
class RateLimiter:
    ...
    async def check_rate_limit(
        ...
        if self.redis:
            ...
            return self._check_local_rate_limit(key, max_requests, window_seconds, current_time)
    ...
    def _check_local_rate_limit(
        ...
        if key not in self.local_cache:
            self.local_cache[key] = {"requests": [], "window_seconds": window_seconds}
...
class RateLimitTier:
    ...
    BASIC = {"requests_per_minute": 30, "requests_per_hour": 500, "requests_per_day": 5000}

    PRO = {"requests_per_minute": 100, "requests_per_hour": 2000, "requests_per_day": 20000}
...
async def rate_limit_dependency(request: Request, max_requests: int = 60, window_seconds: int = 60):
    """
    ...
    try:
        rate_info = await limiter.check_rate_limit(
            key=client_id, max_requests=max_requests, window_seconds=window_seconds
        )
