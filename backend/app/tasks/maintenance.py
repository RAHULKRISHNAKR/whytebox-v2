...
@celery_app.task(bind=True, base=MaintenanceTask, name="app.tasks.maintenance.cleanup_old_results")
...
@celery_app.task(bind=True, base=MaintenanceTask, name="app.tasks.maintenance.cleanup_cache")
...
@celery_app.task(
    bind=True, base=MaintenanceTask, name="app.tasks.maintenance.update_model_statistics"
)
def update_model_statistics(self) -> Dict[str, Any]:
    ...
    try:
        ...
        # Count by framework
        framework_stats = (
            self.db.query(Model.framework, func.count(Model.id)).group_by(Model.framework).all()
        )

        # Count by status
        status_stats = (
            self.db.query(Model.status, func.count(Model.id)).group_by(Model.status).all()
        )
        ...
        stats = {
            ...
            "most_used": [
                {"id": m.id, "name": m.name, "count": m.inference_count} for m in most_used
            ],
        }
        ...
        logger.info("Model statistics updated", extra={"total_models": total_models})
...
@celery_app.task(bind=True, base=MaintenanceTask, name="app.tasks.maintenance.health_check")
def health_check(self) -> Dict[str, Any]:
    ...
    try:
        ...
        if not all_healthy:
            logger.warning("System health check failed", extra={"health_status": health_status})
...
@celery_app.task(bind=True, base=MaintenanceTask, name="app.tasks.maintenance.cleanup_temp_files")
def cleanup_temp_files(self, days: int = 1) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            ...
            extra={"deleted_count": deleted_count, "deleted_size": deleted_size},
        )
...
@celery_app.task(bind=True, base=MaintenanceTask, name="app.tasks.maintenance.backup_database")
def backup_database(self) -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, env={"PGPASSWORD": settings.DATABASE_PASSWORD}
        )
        ...
        logger.info(
            ...
            extra={"backup_file": str(backup_file), "size": backup_size},
        )
