...
@celery_app.task(
    bind=True,
    base=ConversionTask,
    name="app.tasks.conversion.convert_to_tfjs",
    max_retries=2
)
def convert_to_tfjs(
    self,
    ...
    try:
        self.update_state(
            state="PROCESSING",
            meta={"status": "Loading model", "progress": 10}
        )
        ...
        self.update_state(
            state="PROCESSING",
            meta={"status": "Converting to TensorFlow.js", "progress": 50}
        )
...
@celery_app.task(
    bind=True,
    base=ConversionTask,
    name="app.tasks.conversion.optimize_model",
    max_retries=2
)
def optimize_model(
    self,
    ...
    try:
        self.update_state(
            state="PROCESSING",
            meta={"status": f"Applying {optimization_type}", "progress": 20}
        )
