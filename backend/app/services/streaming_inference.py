    ...
    async def run_inference_with_progress(
        ...
        try:
            ...
            await tracker.update(
                step=50,
                stage="preprocessing",
                message="Preprocessing input image"
            )
...
class StreamingExplainabilityService:
    ...
    async def explain_with_progress(
        ...
        try:
            ...
            cache = await get_cache()
            cached_result = await cache.get_explainability(
                model_id, image_data, method, params
            )
            ...
            await tracker.update(
                step=50,
                stage="computing",
                message=f"Computing {method} explanation"
            )
            ...
            if method == "integrated_gradients":
                ...
                for i in range(50):
                    await tracker.update(
                        step=50 + i,
                        stage="interpolation",
                        message=f"Interpolation step {i+1}/50"
                    )
            ...
            await cache.cache_explainability(
                model_id, image_data, method, params, result
            )
            
            await tracker.complete(
                message=f"{method} explanation completed",
                data={"method": method}
            )
