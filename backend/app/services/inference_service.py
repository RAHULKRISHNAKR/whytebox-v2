...
class InferenceService:
    ...
    async def run_batch_inference(
        ...
        if preprocessing_config:
            input_batch = [
                self._preprocess_input(inp, preprocessing_config)
        ...
        if postprocessing_config:
            results = [
                self._postprocess_output(result, postprocessing_config)
    ...
    async def extract_features(
        ...
        # Load model
        engine = self.load_model(
            model_path, framework, device, cache_key=cache_key
        )
        ...
        if hasattr(engine, "extract_features"):
            ...
            raise NotImplementedError(
                f"Feature extraction not supported for {framework}"
            )
