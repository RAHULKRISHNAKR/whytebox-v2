...
from app.tasks import inference, conversion, explainability
...
async def submit_batch_inference_task(
    ...
    try:
        ...
        raise ValidationAPIError(
            message="Failed to submit batch task",
            details={"error": str(e)}
        )
...
@router.post(
    "/conversion/onnx",
    status_code=status.HTTP_202_ACCEPTED,
    summary="Submit ONNX conversion task"
)
async def submit_onnx_conversion_task(
    ...
    try:
        ...
        raise ValidationAPIError(
            message="Failed to submit conversion task",
            details={"error": str(e)}
        )
...
@router.post(
    "/explainability",
    status_code=status.HTTP_202_ACCEPTED,
    summary="Submit explainability task"
)
async def submit_explainability_task(
    ...
    try:
        ...
        raise ValidationAPIError(
            message="Failed to submit explainability task",
            details={"error": str(e)}
        )
...
async def get_task_result(
    ...
    try:
        ...
        if task.state != "SUCCESS":
            raise ValidationAPIError(
                message=f"Task is not complete. Current status: {task.state}"
            )
...
async def cancel_task(
    ...
    try:
        ...
        logger.info(
            ...
            extra={"task_id": task_id, "user_id": current_user.id}
        )
...
@router.post(
    ...
    description="Purge all pending tasks (admin only)"
)
