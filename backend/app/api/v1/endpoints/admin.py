...
from app.core.api_keys import (APIKeyCreate, APIKeyListResponse, APIKeyManager, APIKeyResponse,
                               require_admin)
from app.core.database import get_db
from app.core.error_handlers import ForbiddenAPIError, NotFoundAPIError, ValidationAPIError
    ...
    **Note:** The plain API key is only returned once during creation.
    ...
    if key_data.expires_in_days and key_data.expires_in_days > 365:
        raise ValidationAPIError(
            message="API key expiration cannot exceed 365 days", details={"max_days": 365}
        )
...
async def list_api_keys(
    ...
    keys = manager.list_keys(user_id=user_id, active_only=active_only, skip=skip, limit=limit)

    return APIKeyListResponse(keys=keys, total=len(keys), skip=skip, limit=limit)
...
async def get_api_key(
    key_id: str, db: Session = Depends(get_db), current_user: User = Depends(require_admin)
):
    """
    Get API key details.
...
async def revoke_api_key(
    ...
    logger.info(
        f"API key revoked: {key_id}", extra={"key_id": key_id, "admin_user": current_user.id}
    )
...
async def rotate_api_key(
    ...
    logger.info(
        ...
        extra={"old_key_id": key_id, "new_key_id": result["key_id"], "user_id": old_key.user_id},
    )
...
async def reset_rate_limits(
    ...
    if not user_id and not ip_address:
        raise ValidationAPIError(message="Must provide either user_id or ip_address")
...
@router.get(
    "/config", summary="Get system configuration", description="Get current system configuration"
)
...
async def get_detailed_health(current_user: User = Depends(require_admin)):
    ...
    return {
        ...
        "components": {
            ...
            "cache": {"status": "healthy" if redis_healthy else "unavailable", "type": "redis"},
        },
        "system": {
