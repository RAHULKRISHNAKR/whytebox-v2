...
class TensorFlowInferenceEngine(BaseInferenceEngine):
    ...
    def predict(
        ...
        **kwargs,
        ...
        result = InferenceResult(
            ...
            confidence_scores=(
                confidence_scores[0] if confidence_scores.ndim > 1 else confidence_scores
            ),
    ...
    def predict_batch(
        self, inputs: List[np.ndarray], batch_size: int = 32, return_top_k: int = 5, **kwargs
        ...
        for i in range(0, len(inputs), batch_size):
            ...
            # Run inference
            result = self.predict(batch_array, return_top_k=return_top_k, **kwargs)

            # Split batch results
            for j in range(len(batch)):
                single_result = InferenceResult(
                    predictions=(
                        result.predictions[j] if result.predictions.ndim > 1 else result.predictions
                    ),
                    class_indices=(
                        result.class_indices[j]
                        if hasattr(result.class_indices, "__len__")
                        else result.class_indices
                    ),
                    class_names=(
                        result.class_names[j]
                        if result.class_names and len(result.class_names) > j
                        else None
                    ),
                    confidence_scores=(
                        result.confidence_scores[j]
                        if result.confidence_scores.ndim > 1
                        else result.confidence_scores
                    ),
    ...
    def get_model_info(self) -> Dict[str, Any]:
            ...
            layers.append(
                {
                    ...
                    "output_shape": (
                        str(layer.output_shape) if hasattr(layer, "output_shape") else None
                    ),
                }
            )
        ...
        return {
            ...
            "input_shape": (
                str(self.model.input_shape) if hasattr(self.model, "input_shape") else None
            ),
            "output_shape": (
                str(self.model.output_shape) if hasattr(self.model, "output_shape") else None
            ),
    ...
    def extract_features(self, inputs: np.ndarray, layer_name: Optional[str] = None) -> np.ndarray:
        """
