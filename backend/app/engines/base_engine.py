...
class InferenceResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "class_indices": (
                self.class_indices.tolist() if self.class_indices is not None else None
            ),
            "class_names": self.class_names,
            "confidence_scores": (
                self.confidence_scores.tolist() if self.confidence_scores is not None else None
            ),
...
class BaseInferenceEngine(ABC):
    ...
    @abstractmethod
    def predict_batch(
        self, inputs: List[np.ndarray], batch_size: int = 32, **kwargs
