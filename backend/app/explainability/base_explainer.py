...
class ExplainabilityResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "attribution_map": self.attribution_map.tolist() if isinstance(self.attribution_map, np.ndarray) else self.attribution_map,
            "heatmap": self.heatmap.tolist() if isinstance(self.heatmap, np.ndarray) else self.heatmap,
            "original_image": self.original_image.tolist() if isinstance(self.original_image, np.ndarray) else self.original_image,
...
class BaseExplainer(ABC):
    ...
    def overlay_heatmap(
        ...
        if heatmap.shape[:2] != image.shape[:2]:
            heatmap = BaseExplainer.resize_heatmap(
                heatmap, image.shape[:2]
            )
