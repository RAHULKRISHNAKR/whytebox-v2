...
echo "1. Start WhyteBox (single-port mode — recommended):"
echo "   ${CYAN}./start.sh${NC}"
echo "   Open: ${CYAN}http://localhost:5001${NC}"
echo ""

if [ "$NEEDS_NODE" = false ]; then
    echo "2. Or start in dev mode (hot-reload frontend):"
    echo "   ${CYAN}./start.sh --dev${NC}"
    echo "   Open: ${CYAN}http://localhost:5173${NC}"
    echo ""
    echo "3. Or rebuild frontend then start:"
    echo "   ${CYAN}./start.sh --build${NC}"
    echo ""
    echo "   API Docs: ${CYAN}http://localhost:5001/docs${NC}"
else
    ...
    echo "   Backend API: ${CYAN}http://localhost:5001${NC}"
    echo "   API Docs: ${CYAN}http://localhost:5001/docs${NC}"
fi
