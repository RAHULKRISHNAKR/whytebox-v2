...
function getDims(layer: ArchitectureLayer): Dims {
  ...
  if (t.includes('Pool'))
    return { ...calcConvDims(layer), height: calcConvDims(layer).height * 0.5 };
  ...
  if (flat && flat.length > 0) {
    ...
    for (let py = 0; py < size; py++) {
      for (let pxc = 0; pxc < size; pxc++) {
        ...
        const ny = py / size;
        const wave =
          Math.sin(nx * Math.PI * freq + phase * 2.5) * Math.sin(ny * Math.PI * freq + phase * 1.8);
    ...
    if (flat && flat.length > 0) {
      ...
      for (let py = 0; py < size; py++) {
        for (let pxc = 0; pxc < size; pxc++) {
          ...
          const ny = py / size;
          const wave =
            Math.sin(nx * Math.PI * freq + phase * 2.5) *
...
function createLayerMesh(scene: Scene, layer: ArchitectureLayer, pos: Vector3): Mesh {
  ...
  if (t === 'Flatten') {
    const mesh = MeshBuilder.CreateCylinder(
      `layer_${layer.id}`,
      { height: 0.7, diameter: 0.35, tessellation: 10 },
      scene
    );
    mesh.position = pos.clone();
    const mat = new StandardMaterial(`mat_${layer.id}`, scene);
...
function createConnection(scene: Scene, from: Vector3, to: Vector3, isSkip: boolean): Mesh {
  if (isSkip) {
    ...
    const curve = MeshBuilder.CreateLines(
      `skipc_${Math.random().toString(36).slice(2, 8)}`,
      { points: pts },
      scene
    );
    ...
    const line = MeshBuilder.CreateLines(
      `conn_${Math.random().toString(36).slice(2, 8)}`,
      { points: [from, to] },
      scene
    );
 ...
 */
export function updateAllExpandedActivations(activationData: Record<string, number[][]>): void {
...
// ─── Data Flow Animation API ──────────────────────────────────────────────────
// Animates signal traveling through layers during inference

interface OriginalMeshProps {
...
// Store original properties per layer.id to ensure clean reset
const originalPropsMap = new Map<string, OriginalMeshProps>();

/**
 * Animate layer progress during streaming inference.
 *
 * @param layers - Full architecture layers array
 * @param meshMap - Map of layer.id → Mesh[]
 * @param activeLayerIndex - Current layer being processed (from WebSocket)
 * @param previousLayerIndex - Previous active layer (optional, for transition)
 */
export function animateLayerProgress(
  layers: ArchitectureLayer[],
  meshMap: Map<string, Mesh[]>,
  activeLayerIndex: number,
  previousLayerIndex?: number
): void {
  // Validate inputs
  if (!layers || layers.length === 0 || !meshMap || meshMap.size === 0) {
    console.warn('[animateLayerProgress] Invalid inputs: empty layers or meshMap');
    return;
  }

  if (activeLayerIndex < 0 || activeLayerIndex >= layers.length) {
    console.warn(`[animateLayerProgress] Invalid activeLayerIndex: ${activeLayerIndex}`);
    return;
  }

  // Transition previous layer to "completed" state
  if (
    previousLayerIndex !== undefined &&
    previousLayerIndex >= 0 &&
    previousLayerIndex < layers.length
  ) {
    const prevLayer = layers[previousLayerIndex];
    const prevMeshes = meshMap.get(prevLayer.id);

    if (prevMeshes && prevMeshes.length > 0) {
      prevMeshes.forEach((mesh) => {
        // Store original props on first animation
        if (!originalPropsMap.has(prevLayer.id)) {
          const mat = mesh.material as StandardMaterial;
          if (mat) {
            originalPropsMap.set(prevLayer.id, {
        ...
        // Fade to "completed" state: reduce alpha to 0.6
        const mat = mesh.material as StandardMaterial;
        if (mat) {
          mat.alpha = 0.6;
        }
        ...
        const original = originalPropsMap.get(prevLayer.id);
        if (original) {
          mesh.scaling.x = original.scalingX;
          mesh.scaling.y = original.scalingY;
          mesh.scaling.z = original.scalingZ;
        }
      });
    }
  }
  ...
  const activeMeshes = meshMap.get(activeLayer.id);

  if (!activeMeshes || activeMeshes.length === 0) {
    console.warn(`[animateLayerProgress] No meshes found for layer: ${activeLayer.id}`);
    return;
  }

  activeMeshes.forEach((mesh) => {
    // Store original props on first animation
    if (!originalPropsMap.has(activeLayer.id)) {
      const mat = mesh.material as StandardMaterial;
      if (mat) {
        originalPropsMap.set(activeLayer.id, {
          emissiveColor: mat.emissiveColor.clone(),
          alpha: mat.alpha,
          scalingX: mesh.scaling.x,
          scalingY: mesh.scaling.y,
    ...
    const mat = mesh.material as StandardMaterial;
    if (mat) {
      // Increase emissive color for "active" glow
      mat.emissiveColor = new Color3(0.3, 0.6, 1.0); // Bright blue glow
      mat.alpha = 1.0; // Full opacity
    }
...
/**
 * Reset all layer animations to original state.
 * Call this when inference completes or is cancelled.
 *
 * @param meshMap - Map of layer.id → Mesh[]
 */
export function resetLayerAnimations(meshMap: Map<string, Mesh[]>): void {
  if (!meshMap || meshMap.size === 0) {
    console.warn('[resetLayerAnimations] Invalid meshMap');
    return;
  }

  originalPropsMap.forEach((original, layerId) => {
    const meshes = meshMap.get(layerId);
    if (!meshes) return;

    meshes.forEach((mesh) => {
      const mat = mesh.material as StandardMaterial;
      if (mat) {
        mat.emissiveColor = original.emissiveColor.clone();
        mat.alpha = original.alpha;
      }
  ...
  // Clear the map after reset
  originalPropsMap.clear();
}

// ─── Layer Contribution Colors API ───────────────────────────────────────────
// Apply Grad-CAM contribution scores as mesh colors

/**
 * Apply layer contribution colors based on Grad-CAM gradient magnitudes.
 *
 * Color mapping:
 * - 0.0-0.33 → cool blue (default/low contribution)
 * - 0.34-0.66 → orange (medium contribution)
 * - 0.67-1.0 → red (high contribution)
 *
 * @param layers - Full architecture layers array
 * @param meshMap - Map of layer.id → Mesh[]
 * @param contributions - Layer contribution scores (0.0-1.0) keyed by layer name
 */
export function applyLayerContributions(
  layers: ArchitectureLayer[],
  meshMap: Map<string, Mesh[]>,
  contributions: Record<string, number>
): void {
  if (!layers || layers.length === 0 || !meshMap || meshMap.size === 0) {
    console.warn('[applyLayerContributions] Invalid inputs');
    return;
  }
  ...
  layers.forEach((layer) => {
    ...
    // Match layer name to contribution score
    // Backend returns layer names like "features.0", "classifier.1", etc.
    // Layer.name might be "features.0" or just the layer type
    const score = contributions[layer.name];

    if (score === undefined) {
      // Layer not in contributions dict - keep default color
      return;
    }
    ...
    let color: Color3;
    if (score < 0.34) {
      // Low contribution: cool blue (default)
      color = Color3.FromHexString('#4A90E2');
    } else if (score < 0.67) {
      // Medium contribution: orange
      color = Color3.FromHexString('#FF9800');
    } else {
      // High contribution: red
      color = Color3.FromHexString('#F44336');
    }

    // Apply color to all meshes for this layer
    meshes.forEach((mesh) => {
      const mat = mesh.material as StandardMaterial;
      if (mat) {
        mat.diffuseColor = color;
        mat.emissiveColor = color.scale(0.2); // Subtle glow
      }
    });
  });
}
...
/**
 * Clear layer contribution colors and restore default colors.
 *
 * @param layers - Full architecture layers array
 * @param meshMap - Map of layer.id → Mesh[]
 */
export function clearLayerContributions(
  layers: ArchitectureLayer[],
  meshMap: Map<string, Mesh[]>
): void {
  if (!layers || layers.length === 0 || !meshMap || meshMap.size === 0) {
    console.warn('[clearLayerContributions] Invalid inputs');
    return;
  }
  ...
  layers.forEach((layer) => {
    ...
    // Restore default color based on layer type
    const defaultColorHex = LAYER_COLORS[layer.type] || '#95A5A6';
    const defaultColor = Color3.FromHexString(defaultColorHex);

    meshes.forEach((mesh) => {
      const mat = mesh.material as StandardMaterial;
      if (mat) {
        mat.diffuseColor = defaultColor;
        mat.emissiveColor = defaultColor.scale(0.15);
      }
    });
  });
}
